...
#X = df_pca.drop("patient_id", axis = 1)
y = df_pca["RandN_BMS"]
...
param_grid = {'solver': ['adam'],
                  'alpha': [0, 1e-02, 1e-04, 1e-06],  # L2 reg
                  'activation': ['relu'],
                  'hidden_layer_sizes': [ (16,8), (32,16,8), (64,32,16,8), (128,64,32,16,8)],
...
#enumerate splits
fold_no = 1
outer_results_auc_MLP = list()
outer_results_auprc_MLP = list()
for train_ix, test_ix in cv_outer.split(X,df_pca.stratify_vars):
    ...
    train.to_csv('train_fold_year9_MLP_primary_exclAW_worst_' + str(fold_no) + '.csv')
    test.to_csv('test_fold_year9_MLP_primary_exclAW_worst_' + str(fold_no) + '.csv')
    fold_no += 1
    ...
    #execute search
    result = search.fit(X_train_scaled.values, y_train)
    ...
    #store result
    outer_results_auc_MLP.append(roc_auc)
    outer_results_auprc_MLP.append(auprc)
    # report progress
    print('>auc=%.3f, est=%.3f, bp=%s' % (roc_auc, result.best_score_, result.best_params_))

import pickle
with open("auc_MLP_year9_worst_exclAW", "wb") as fp:
    pickle.dump(outer_results_auc_MLP, fp)
    
with open("auc_MLP_year9_worst_exclAW", "rb") as fp:
    b= pickle.load(fp)

import pickle
with open("auprc_MLP_year9_worst_exclAW", "wb") as fp:
    pickle.dump(outer_results_auprc_MLP, fp)
    
with open("auprc_MLP_year9_worst_exclAW", "rb") as fp:
    b= pickle.load(fp)     
    

mean(outer_results_auc_MLP), std(outer_results_auc_MLP) 
mean(outer_results_auprc_MLP), std(outer_results_auprc_MLP)
rel_auprc = [x/0.166 for x in outer_results_auprc_MLP]
